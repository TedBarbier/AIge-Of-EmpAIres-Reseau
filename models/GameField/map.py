...
class Map:
    ...
    def __init__(self,_nb_CellX , _nb_CellY):
        ...
        self.state = "normal"
        self.seed = None # Initialize seed attribute
    ...
    def generate_map_multi(self, gen_mode=MAP_NORMAL, mode=MARINES, selected_player=3, team=1, polygon=None, ai_config=None, network_manager=None):
        # Set a specific seed if provided, otherwise generate one
        if self.seed is None:
            self.seed = random.randint(0, 100000)
        ...
        polygon = self._place_player_starting_areas_multi(mode, selected_player, team ,team, polygon, ai_config, network_manager=None)
    ...
    def _place_player_starting_areas_multi(self, mode, selected_player, num_players, team=1, polygon=None, ai_config = None, network_manager=None):
        if team not in self.players_dict:
            ...
            current_player = Player(center_Y, center_X, team, num_players, True, ai_config, network_manager=None)
            current_player.linked_map = self
            self.players_dict[current_player.team] = current_player
            ...
            current_player_resources = gen_option.get("resources").copy()
            current_player.add_resources(current_player_resources)
            return polygon
        else:
            return None
